...
async function carregarContratos(selectId, selectedId = null) {
    ...
    try {
        const response = await fetchComAuth('/api/contrato');
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (modalCriarLPU) {
        ...
        formCriarLPU.addEventListener('submit', async function (event) {
            ...
            try {
                const response = await fetchComAuth('/api/lpu', {
                    method: 'POST',
    ...
    if (modalAlterarLPU) {
        ...
        document.getElementById('selectLPUAlterar').addEventListener('change', async function (event) {
            ...
            try {
                ...
                const lpuResponse = await fetchComAuth(`/api/lpu/${lpuId}`);
                ...
                const contratosResponse = await fetchComAuth('/api/contrato');
        ...
        formAlterarLPU.addEventListener('submit', async function (event) {
            ...
            try {
                ...
                const response = await fetchComAuth(`/api/lpu/${lpuId}`, {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json' },
